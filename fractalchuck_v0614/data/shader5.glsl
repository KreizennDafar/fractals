...
//http://glsl.heroku.com/e#10952.1
...
{
	float thick = 0.5 - (0.5 + sin(uv.x*8.*pi - mouse.x*0.1)*0.5)*0.25;
	...
	return line*color_var;
	//return line*vec3(0.30, 0.59, 0.11);
}
...
void main(void)
{
	...
	uv_bipolar = spiralzoom(uv_bipolar, vec2(0.), 1., pi/5., 0.8, -mouse.y*vec2(8.,0.) + mouse.x*vec2(-1,1)*20.);
	uv_bipolar = vec2(-uv_bipolar.y,uv_bipolar.x); // 90° rotation 
	
	vec2 uv_trace = fract(uv_bipolar);
	uv_trace.x -= (uv_trace.y - 0.5)*1.25;

	gl_FragColor.xyz = tracerline(uv_trace);
