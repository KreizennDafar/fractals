...
//from http://glsl.heroku.com/e#14352.0
...
float getLevel( vec2 pos ){
	...
	return floor(log2(distance) - time*(SPEED-0.2*mouse.y));
}

float getLevelSize( float level){
	return exp2(level + time*(SPEED-0.2*mouse.y));
}
...
vec3 getTriangleColor( float level, float offs, vec3 back){
	float alpha = pow(0.5, offs);
	float t = offs/3.4696 - level/20.2363 ;+ time*0.1;//getLevelSize(level)/length(resolution.xy);
...
void main( void ) {
	...
	float theta = atan(position.y, position.x) + 100.*mouse.x*ROTSPEED;
